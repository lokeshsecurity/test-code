			...
			link: function(scope, element, attrs) {
				...
				var smartActions = {
				    ...
				    userLogout: function($this){
						...
						// ask verification
						$.SmartMessageBox({
							title : "<i class='fa fa-sign-out txt-color-orangeDark'></i>هل تريد الخروج من النظام؟  ",
							...
							if (ButtonPressed == "نعم") {
								...
								$rootScope.$broadcast('disConnectSocket'); //disconnect websocket from mainController
							}
						});
						function logout() {
...
/**
 * remove the div and add ng-include
 */
	.directive('activityContent', ['$compile', function($compile) {
		return {
			...
			template: '\
				<div class="ajax-dropdown" style="margin-left:5.4% !important">\
...
//				</label>',
			link: function(scope, element, attrs, parentCtrls) {
				...
				console.log('contentCtrl : ' + contentCtrl);
